...
const ModifyModal = ({ dispatch, actionRef, loading, announcementData, enums }) => {
  ...
  const showModal = (id: any, state: any, t: any) => {
    ...
    if (t === 'receive') {
      dispatch({
        type: 'documentMgt/getReceiveDetail',
  ...
  useEffect(() => {
    ...
    if (query.get('type') === 'modify' && query.get('id')) {
      showModal(query.get('id'), query.get('status'), 'publish');
    }
  }, []);
  ...
  useEffect(() => {
    if (announcementData) {
      ...
      //   });
      setVisibleRange(announcementData.visibleRange);
  ...
  const hideModal = (): void => {
    dispatch({
      type: 'documentMgt/removeAnnouncementDetail',
    });
      ...
      ? form
          ...
          .then((values: any) => {
            ...
            return new Promise(resolve => {
              dispatch({
                type: `documentMgt/auditAnnouncement`,
                payload: {
                  ...values,
                  noticeId,
                },
    ...
    >
      <Spin spinning={loading}>
        <Descriptions title="基本信息" column={{ xxl: 3, xl: 3, lg: 3, md: 2, sm: 2, xs: 1 }}>
          ...
          {fileList(announcementData.fileList)}
        </Descriptions>
        {createItem()}
...
export default connect(({ loading, documentMgt, global }) => ({
  loading: loading.models.documentMgt,
  announcementData: documentMgt.announcementData,
  enums: global.enums,
}))(ModifyModal);
