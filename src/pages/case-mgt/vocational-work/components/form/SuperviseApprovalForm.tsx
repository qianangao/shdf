import React, { useEffect } from 'react';
import AdvancedForm from '@/components/AdvancedForm';
import { Descriptions } from 'antd';

const CaseMgt = ({ form, orgInfoData }) => {
  const formItems = [
    {
      label: '备注',
      name: 'approvalOpinion',
      span: 4,
      type: 'textarea',
      rules: [
        { message: '请输入备注!', whitespace: true },
        { max: 80, message: '备注长度请小于80位!', whitespace: true },
      ],
    },
  ];

  useEffect(() => {
    if (orgInfoData) {
      form.setFieldsValue({});
    }
  }, [orgInfoData]);

  const selectLgbInput = (
    // 显示单位信息-公共组件
    <>
      <Descriptions size="middle" column={2} title="督办申请">
        <Descriptions.Item label="申请时间">{orgInfoData.createTime}</Descriptions.Item>
        <Descriptions.Item label="申请人">{orgInfoData.createUser}</Descriptions.Item>
      </Descriptions>
      <Descriptions size="middle" column={1}>
        <Descriptions.Item label="申请备注">{orgInfoData.applyRemarks}</Descriptions.Item>
      </Descriptions>

      <Descriptions size="middle" column={2} title="督办审批">
        <Descriptions.Item label="审批时间">{orgInfoData.lastUpdateTime}</Descriptions.Item>
        <Descriptions.Item label="审批人">{orgInfoData.approvalUser}</Descriptions.Item>
        <Descriptions.Item label="开始时间">{orgInfoData.superviseStart}</Descriptions.Item>
        <Descriptions.Item label="结束时间">{orgInfoData.superviseEnd}</Descriptions.Item>
      </Descriptions>
    </>
  );

  return <AdvancedForm form={form} fields={formItems} headerRender={selectLgbInput} />;
};

CaseMgt.useForm = AdvancedForm.useForm;

export default CaseMgt;
